...
Still open, all C++-side and out of this phase's budget: view modes (Transpose/Text-via-UI/Record tree, `recordRows`, the column-visibility popup, `result_view_modes.{h,cpp}`) and the aggregate footer UI (`aggregateExact`/`aggregateComputed` wiring, including the "fetched rows only" fallback label for a statement `aggregate_query` cannot wrap — that fallback needs a Rust-side decision this phase did not add, since `run_aggregate` today just reports whatever error the wrapped query hits rather than distinguishing "not wrappable" from any other failure). F4.4's create/modify object dialogs remain untouched.
