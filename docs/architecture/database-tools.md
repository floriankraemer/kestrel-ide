...
- **Cassandra/Scylla** (feature `cassandra`, **opt-in only, not in `default`**) — async, `scylla` 1.9 (`rustls-023`). Introspects `system_schema.{keyspaces,tables,views,types,functions,columns}` directly. Statements split by `db_sql::split` (`Dialect::Cassandra`, semicolon-based), classified read/write by leading keyword. `cassandra` is opt-in because `scylla`'s own `Cargo.toml` enables rustls's default `aws_lc_rs` feature with no per-dependent override — pulling it into `default` would fail the workspace's `aws-lc-rs` gate for every consumer, not just the few who need it (`crates/db-drivers/Cargo.toml`'s `[dependencies.scylla]` comment has the full account, verified against the published manifest for 1.7.0/1.8.0/1.9.0).
