...
| Task | Status | Commit |
|---|---|---|
| FZ.1 — `db-drivers::mysql` (feature `mysql`, default on): `mysql_async` 0.37, `rustls-tls`+`ring`+pure-Rust `flate2` (no `aws-lc-rs`, no C zlib); connect/execute/streaming/cancel (`KILL QUERY` on a second connection)/introspection at all three levels/`ddl_of` via `SHOW CREATE`; `mysql`/`mariadb` `database-drivers` manifest rows (same native id, family `mysql`) and a `mysql` `sql-dialects` row + keyword asset; `plugin-host`'s row-count assertions updated | done | — |
| FZ.2 — real-server tests: `docker/db-compose.yml` gains `mysql:8.4`/`mariadb:11`; `make test-db`'s `db-ci` exercises both (connect/execute/introspect, a 1M-row recursive-CTE stream, `KILL QUERY` cancel of a `SELECT SLEEP(30)`) | done | — |
