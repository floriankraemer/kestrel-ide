...
**Exception: Cassandra/Scylla has no TLS mode at all (F7.4).** The `scylla` crate's own `Cargo.toml` gives a dependent no way to select rustls's `ring` provider over its default `aws_lc_rs` one — unlike every other native driver in this table, whose crates each expose that choice explicitly — so `db-drivers::cassandra` never enables scylla's TLS feature in any configuration. `SslMode` other than `Disable` returns `NotSupported` for this one backend, rather than either connecting without verification or letting `aws-lc-rs` into the tree. This is a permanent property of the current dependency, not a "not yet" deferral like Postgres's own F1 TLS gap elsewhere in this codebase; revisit if `scylla` ever exposes a provider-agnostic rustls feature.
