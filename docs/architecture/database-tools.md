...
crates/db-drivers/src/   lib.rs  postgres.rs  sqlite.rs  sqlite_bench.rs  sqlite_tests.rs  mongodb.rs  redis.rs  cassandra.rs
                         introspect/*.sql  testsupport.rs  ssh.rs (MySQL/MariaDB not yet built, still F8 follow-up — see §3)
...
Idle-close (30 minutes) is still not implemented — every connected source's worker stays alive until `disconnectSource` is called by hand; this is real, unclaimed follow-up debt (see §11), not merely deferred alongside work that has since landed (the SSH tunnel and TLS wiring `SessionWorker` once lacked are both wired in now, F7c/FX).
...
`idle_close_minutes` exists as a persisted setting (§8), but no code reads it back to actually close an idle connection yet — see the idle-close debt row just above and in §11; a connection today only ever closes on an explicit `disconnectSource` or app exit.
...
|---|---|---|
| `idle_close_minutes` is a persisted setting with no code behind it — every connected source's `SessionWorker` stays alive until `disconnectSource` is called by hand or the app exits | No phase has built the actual timer/close path; the setting was added early (F1/F2) as the intended knob | A user with many idle long-lived connections reports the resource cost, or a phase revisits `SessionWorker` for another reason |
...
| `db_core::error::DbErrorCode` has no `DriverQuarantined` variant; the quarantine module uses `ConnectionFailed` as the closest existing code | The enum is append-only, so this is additive, not a rename; no caller has needed to distinguish the two yet | A caller needs to tell "quarantined" and "connection failed" apart in the UI |
...
| `database_panel.cpp` uses `QStyle` standard icons, not a themed mask set; no colour-tag bar; no speed-search; one Flat/Grouped toggle rather than a full grouping menu; an object action's confirmation dialog states the action in English rather than showing the exact SQL `db_core::ddl` generated; a successful rename/drop/truncate/comment does not auto-refresh the affected node | Cosmetic/UX polish beyond the parity bar this phase targeted; every affordance still works, just not to the fullest JetBrains-parity finish | A real user/design request for the polish pass |
