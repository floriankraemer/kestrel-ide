...
fn first_row(connection: &mut dyn db_core::driver::Connection, sql: &str) -> Result<Vec<Value>, String> {
...
fn run_aggregate(
    ...
    let data_source = db_core::datasource::DataSource::from(&setting);
    let spec =
        db_core::datasource::ConnectSpec::from(&data_source, &super::service::secrets_for(source_id));
    ...
    let row = outcome?;
    let value = row.first().cloned().unwrap_or(Value::Null).display(&Default::default());
    ...
    for constraint in table_constraints {
        ...
        targets.push(ffi::FfiDbNavTarget {
            ...
            statement: QString::from(
                format!("SELECT * FROM {table_ref} WHERE {}", conditions.join(" AND ")).as_str(),
            ),
        });
    }
    targets
...
impl ffi::ConsoleService {
    ...
    pub fn aggregate_exact(
        ...
        std::thread::spawn(move || {
            let outcome = run_aggregate(&source_id, &statement_text, &column_for_thread, op, dialect);
            let _ = qt_thread.queue(move |mut service: Pin<&mut ffi::ConsoleService>| {
                match outcome {
                    Ok((value, count)) => service.as_mut().aggregate_computed(
                        result_id,
                        QString::from(column_for_thread.as_str()),
                        op,
                        true,
    ...
    pub fn go_to_nav_target(
        ...
        match shared.consoles[&tab_id].worker.send(SessionCommand::Execute {
            statement: DbStatement::sql(statement),
            options: ExecOptions {
                fetch_size: page_size,
                max_rows: None,
...
mod tests {
    ...
    fn aggregate_query_wraps_the_statement_and_always_counts_rows() {
        let sql = aggregate_query("SELECT * FROM t", "amount", ffi::FfiDbAggOp::Sum, Dialect::Sqlite);
        assert_eq!(
            sql,
    ...
    fn aggregate_query_count_ignores_the_column_and_still_counts_rows() {
        let sql = aggregate_query("SELECT * FROM t", "id", ffi::FfiDbAggOp::Count, Dialect::Sqlite);
        assert_eq!(sql, "SELECT COUNT(\"id\"), COUNT(*) FROM (SELECT * FROM t) t");
    }

    fn fk_constraint(column: &str, ref_table: &str, ref_column: &str) -> db_core::schema::ConstraintKind {
