...
impl Quarantine {
    ...
    pub fn check(&self, driver_id: &str) -> Result<(), DbError> {
        if self.marker_path(driver_id).exists() {
            // `db_core::error::DbErrorCode` has no `DriverQuarantined`
            // variant yet (it is append-only, F1's crate, out of scope
            // for this phase to edit) — `ConnectionFailed` is the closest
            // existing code; see this crate's F8a report for the
            // requested addition.
...
mod tests {
    ...
    fn arming_then_checking_reports_quarantined() {
        ...
        assert_eq!(err.code, DbErrorCode::ConnectionFailed);
