...
cargo tree -p container-core -e normal | grep -i qt              # must be empty
cargo tree -p container-core -e normal --depth 1 | grep -i tokio # must be empty (tokio appears deeper only as reqwest's own internal runtime, the same as for ai-chat-core; container-core code never touches it)
```
