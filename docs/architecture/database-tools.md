...
**Phase FY**: closed the icon/colour/view-options/confirmation-refresh F2 polish debt and the bracket-skip bug FX found (both described in their own places above — §11's F2/F3 debt rows), and extended `e2e_containers_settings_test_connection` (G1.6, still 14 flows total) to disable the `containers` plugin from the Settings dialog and relaunch, asserting the View menu's `Containers` action is gone — new `plugins_page_rows`/`plugins_page_toggle` e2eMarks (`plugins_page.cpp`) and a `plugins_category_rect` on `settings_dialog.cpp`'s own `dialog_shown` mark. This flow could not be confirmed green under `../mk e2e-repeat` in this phase's own sandbox — it timed out waiting for `main_window_shown` even reverted to its pre-existing, unmodified form, while every other flow in the same file passed reliably, so the cause sits in this shared sandbox's own resource contention rather than in the change; the main session's wave-end E2E pass is where this gets its real green run.
...
Postgres TLS **closed by phase FX**: `db-drivers::postgres::connect` now dispatches `Prefer`/`Require`/`VerifyCa`/`VerifyFull` through `tokio_postgres_rustls::MakeRustlsConnect` (`tls_config_for`) — `VerifyFull` verifies the full chain and hostname against `ca_file`'s PEM roots or (no `ca_file`) the OS trust store (`rustls-native-certs`); `VerifyCa` is handled identically to `VerifyFull` in this pass (a chain-only, no-hostname verifier needs a deeper corner of `rustls::client::danger` than this pass budgeted for — documented as a conscious "stricter than asked, not a silent gap" simplification, not a TODO); `Prefer`/`Require` use a named `DangerousNoVerify` `ServerCertVerifier` (encrypted, deliberately unverified, `ring` crypto provider explicit throughout — `cargo tree --all-features -i aws-lc-rs` stays empty). Unit-tested (`postgres.rs`): config-mapping success for every mode, a missing `ca_file` reported not panicked, a garbage `ca_file` (0 certificates parsed) reported not panicked. No `db-integration` test against a real server yet — the compose Postgres does not currently serve TLS; trigger: a compose image that does, or a real user report against a live server.
`db-driver-adbc`/`db-driver-odbc` do not honour `IntrospectLevel` (see §6).
**Closed by phase FY**: `database_panel.cpp` now paints a `.a8` mask per kind (`resources/icons/database/`, 12 glyphs — database/schema/table/view/column/key/index/routine/folder/collection/keyspace/redis-key, `maskForIconKey`'s lookup table replacing every `QStyle` standard icon; a primary-key column gets the dedicated key glyph via a new `TreeRow`/`FfiDbTreeRow::primary_key` bool threaded from `NodeDetail::primary_key`) and a source's own colour as a left-edge gradient on column 0 (and as a swatch on the console bar's source combo); the Flat toggle is now a "View options" menu (Group by Object Type/Flat, Separate Procedures and Functions, Sort Natural/Alphabetical — `setSeparateRoutines`/`setSort`, new `DatabaseService` qinvokables over `db_core::tree`'s existing knobs) and Esc clears the speed-search filter edit (the filter itself — `setFilter`'s pattern syntax — already existed since F2.2); rename/drop/truncate/comment confirmations now show the exact statement `DatabaseService::actionPreview` generates (factored out of `runAction`'s own dispatch) instead of a generic English sentence, and the affected scope (the row's parent for rename/drop, the row itself for truncate/comment) auto-refreshes on success, generalising F4.4's `ddlRefreshNodeId_` precedent.
"Jump to console" is a `qWarning` stub, per-plan (F3 wires it, "Open Console" closed in phase FX — see §10).

**F3 debt** (all tracked in `database-tools-plan.md`'s F3 row, not silently dropped):
`e2e_database_console.rs` **landed in phase FX** — see §10 for the flow, the numbers, and what it still leaves out (grid paging beyond one sample, the busy-query cancel sub-flow).
**Closed by phase FY**: the bracket-skip bug FX's own console work found (typing `)` after an auto-inserted `)` inserted a second one whenever the tracked closer had been forgotten) — `edit_ops::pairs::PairTracker::type_char` now types over an identical non-quote closing delimiter under the caret whether or not this session inserted it (IntelliJ's own rule), not only a session-tracked one; quotes are unchanged (tracked-only, since the same character opens and closes a string). Unit-tested in `pairs.rs`. The busy-query cancel sub-flow itself is still not click-driven end to end — see below.
...
**F7c debt** (paid off: the SSH tunnel wiring and dialog relabel-only rows F7b left above — see §4/§9's implementation notes; not silently dropped, the rest below is what F7c itself left open):
A Cassandra contact-points *list* is still not built — the dialog's single `host` field carries one comma-free contact point, and multi-contact-point support needs a real `List`-kind field this phase's own three extra fields (all `Text`/`Bool`) did not need to build. Trigger: a real multi-contact-point Cassandra source is configured through this dialog and one host proves insufficient (Scylla/Cassandra tolerate a single seed node for the initial handshake, so this is not a hard blocker, only a convenience gap).
No generic form-builder (`data_source_form.{h,cpp}`) exists for the dialog's *common* fields (host/port/database/user/ssh/ssl) — only the family-specific "Options" extras go through a descriptor; the plan's fuller ask (a generic `FfiFormField{key,label,kind,section,default,hint}` vocabulary covering every field, ADBC/ODBC included per F8b) is deliberately not built, since the concrete gap this phase found (Mongo/Redis/Cassandra's three missing fields) did not need it — building a generic engine for fields that already work as named setters would be speculative machinery with nothing yet to justify it (YAGNI). Trigger: F8b's ADBC/ODBC dialog work, or a new family whose *common* fields (not just its extras) genuinely differ from the existing host/port/database/user shape.
`db-integration`'s sshd-backed connect test (F7a's own addition, `db_drivers::ssh`'s `#[ignore]`d `tunnels_through_the_compose_sshd_to_postgres`) was not re-run against a real tunnel this phase — no `docker/db-compose.yml` sshd service reachable in this sandbox; `make test-db` is the trigger once that compose service exists and a session has network access to run it.
