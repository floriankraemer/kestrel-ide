...
|---|---|---|---|
| Data sources | 30+ complete / 20+ basic DBMS; SSH/SSL/proxy; password/Kerberos/IAM/pgpass; read-only; groups + colour; project vs global; driver auto-download; `dataSources.xml` without passwords | **As delivered**: native PostgreSQL, SQLite, MongoDB, Redis, Cassandra/Scylla (MySQL/MariaDB not built — still open, see database-tools.md §3). ADBC with a pinned, IDE-managed download: DuckDB, Snowflake, BigQuery. ADBC/ODBC `install-hint` only (no pinnable artifact found, F8.3 spike): SQL Server, ClickHouse, Trino. ODBC generic row: any system DSN (Oracle/Databricks/Presto/HANA/Teradata/Redshift/Spark/SingleStore/Exasol/Druid/Flight SQL are reachable this way if a system driver exists, not their own manifest rows). SSH tunnel (CLI first, agent/key/config/password/ProxyJump; in-process `russh` fallback for password auth or no CLI), TLS modes (Postgres real; Cassandra plaintext-only), password auth; read-only; groups + colour; global/project TOML; secrets in keychain. Kerberos/IAM/proxy deferred. | F1, F7, F8 |
...
`database-tools.md` §2 is the truth-up against the code as it actually shipped — read that one for the real file list; two known-stale details here: `db-drivers` never grew a `mysql.rs` (MySQL/MariaDB is still unbuilt) or its own `tunnel.rs` (the real file is `ssh.rs`, holding `RusshTunnel`; `Tunnel`/`CliTunnel` live in `db-core::tunnel` as designed).*
...
- Nightly/on demand (`docs/architecture/db-integration.md`): no dedicated `linux-db` image stage yet — `make test-db` reaches `docker/db-compose.yml`'s services (`postgres:17`, `mongo:8`, `redis:7`, `scylladb/scylla:2026.2`, `linuxserver/openssh-server` for the SSH tunnel test) from inside `linux-builder` itself over `--network host`; `db-ci` = `cargo nextest run -p db-drivers --features db-integration` plus a second `--run-ignored only ssh` pass for the sshd-backed tunnel test. MySQL/MariaDB and SQL Server (`mssql/server:2022`) have no compose service yet, `db-driver-adbc`/`db-driver-odbc` are not yet wired into `db-ci`, `.github/workflows/nightly.yml` has no `db-integration` job yet, and `make e2e-db` does not exist — all four remain open follow-up work, not shipped.
...
Not delivered, tracked as open follow-up rather than silently dropped: MySQL/MariaDB (no driver crate exists; R2's `aws-lc-rs` audit and the rest of the seam are otherwise ready for it), a dedicated `linux-db` Docker image stage and `make e2e-db`, the MySQL/MariaDB and SQL Server nightly compose services, and a from-scratch Windows cross-build of the tree this plan actually shipped (§13 is P0's own spike only — see the Windows note below). Every other gap is recorded, with its own trigger, in `database-tools.md` §11's consolidated debt table.
