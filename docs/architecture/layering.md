...
- **Which kind of page a tab needs** is `app-core`'s answer, carried across the seam as `TabKind` (ADR-0020), now `Text`/`Binary`/`Diff`/`Image` (issue #258). The view builds a `CodeEditor`, a `HexViewer` or an `ImageViewer` from it and never infers the kind from the path or the bytes. What a hex row *says* — offset format, byte grouping, printable-byte rule, short-row padding — belongs to `editor_core::hex`, next to the `binary_detect` rule that decides what counts as binary in the first place.
