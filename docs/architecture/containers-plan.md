...
`RunConfigSetting.kind` + `ContainerImageRunSetting`/`ContainerfileRunSetting`/`ComposeRunSetting` (every JetBrains option from the three tables, same names); `run_config.rs` argv compilers + `preview()`; `run-core`: `to_launch_spec` dispatch by kind (`program`/`args` derived from the sub-table, never stored), Containerfile = auto-added `BeforeLaunchTask::ExternalTool{docker build …}` + `docker run …` launch (build output in the Build dock, the IDE's before-launch convention), compose `down`/`stop` counterparts as actions on the running console; `run_config_dialog.cpp` gains a kind picker + three pages with "Modify options" disclosure and live command preview; run toolbar/menu entries unchanged (they already list configs); Compose tree nodes (Start All / Stop / Down / Scale dialog / Jump to Source via `config_files` label); Build Log tab = the run console; Dockerfile gutter icon (`config_for_file` → containerfile config; popup Build / Run / New configuration…); ADR-0056 (extends ADR-0039).
Tests: table-driven argv for all options x2 engines, preview string, Containerfile build→run sequencing, compose services picker via `compose config --services` (stubbed), gutter config derivation.
...
- Layering: `cargo tree -p container-core -e normal | grep -iE 'qt|tokio'` empty; same for `run-core` once C5 adds the edge.
...
| C3 | Container actions and tabs | done | `pending` |
