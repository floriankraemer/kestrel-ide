...
/// — `FfiImportPreview::sampleRows` joins each sample row the same way
/// `FfiDbRow::cells` does, so this must stay the same character;
/// duplicated rather than imported because that module's constant is
/// private (F4a's own file, not touched by this phase). Export ("Copy
/// as"/"Export…") no longer uses this separator at all as of F4c — rows
/// cross the seam as typed JSON (`ResultProvider::rowValues`) instead.
...
/// Decodes `rows` — `ResultProvider::rowValues`'s own JSON-per-row
/// encoding (`db_core::value::row_to_json`), one `QString` per row — back
/// into a typed [`RowBatch`], never through `Value::Text` the way the old
/// `FfiDbRow`-backed version of this function had to (F4c, closing F5b's
/// gap 4: a grid export now types every cell exactly as the driver
/// reported it). A row that fails to decode (should not happen — this
/// crate is the only producer of the encoding) becomes a row of `Value::
/// Null`s rather than aborting the whole export.
fn parse_typed_rows(columns: &QStringList, rows: &QStringList) -> (Vec<ColumnMeta>, Vec<RowBatch>) {
    let column_metas: Vec<ColumnMeta> = (0..columns.len())
        ...
        .collect();
    let width = column_metas.len();
    let parsed_rows: Vec<Vec<Value>> = (0..rows.len())
        .map(|i| {
            let text = rows.get(i).map(|q| q.to_string()).unwrap_or_default();
            db_core::value::row_from_json(&text).unwrap_or_else(|_| vec![Value::Null; width])
        })
        .collect();
    let batch = RowBatch {
...
fn render_rows(
    ...
    let (column_metas, batches) = parse_typed_rows(columns, rows);
