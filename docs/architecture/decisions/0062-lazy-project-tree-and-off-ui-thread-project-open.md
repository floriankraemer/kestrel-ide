...
Accepted and delivered for trimming the `projectOpened` slot chain itself (this document's "Decision 3") in PR3 of the same plan.
...
### 3. Trim `projectOpened`'s own UI-thread work (PR3, delivered)

Painting first (Decision 1) and loading lazily (Decision 2) move everything *before* `projectOpened` off the Qt thread, but every slot the signal itself fans out to — search, LSP, build tools, the analysis status label, the database dock — still ran its own settings read synchronously right after: roughly seven to nine `settings.toml`/`.ide/settings.toml` parses per open, one per listener, for what is the same answer every time.

**Shared, invalidated cache instead of `AppSession`.** `app-config` gains `resolved_cache` (`crates/app-config/src/resolved_cache.rs`): one cached `Settings` for the global file, one cached `(ProjectSettings, Settings)` pair for whichever project root was last resolved. `app-config` cannot depend on `settings-model` (layering.md: `settings-model` sits above it and owns `scope::resolve`'s precedence rule, ADR-0017/ADR-0022), so the cache does not compute the resolved answer itself — it takes a loader closure and remembers what that closure returned, the same shape `std::sync::OnceLock`/`HashMap::entry` already give "compute once, reuse after" elsewhere in the standard library. `app_config::save`/`update` and `project_settings::save`/`update` call `resolved_cache::invalidate()` on every successful write, so the settings dialog's save path — every one of them funnels through those four functions — can never leave a stale answer cached; this is the ADR-0037 rule this cache must not break, restated for the cache rather than for a one-off parse.

`ui-shell`'s `convert.rs` already had the single funnel every consumer of a resolved value calls through — `load_settings`, `load_project_settings`, `load_resolved_settings`, `load_resolved_settings_for` — so wiring the cache in at that one seam is what makes every listed consumer (search, LSP, build tools, the database dock, the analysis label, `ProjectTreeModel`'s own `Default`/`apply_remote_wsl_setting`) share one parse without touching each call site's own logic. `load_resolved_settings_for(root)` (the explicit-root form `SearchModel::open_index`'s worker thread already used off the Qt thread, ADR-0037 §Alternatives) reads and populates the same cache entry `load_resolved_settings()` does on the Qt thread — whichever of them runs first after a project opens does the one real parse, the rest hit the cache.

**Edits from outside this process are caught by stamp, not by the watcher.** A hand edit, a `git checkout`, or a second IDE instance saving its recents bypasses the four save functions, and the global `settings.toml` is not under any project watcher at all.
So every cache entry records the modification time and length of each file it was built from, and every hit re-stats them (one `stat` per file, microseconds against a TOML parse) and misses on any difference, including a file appearing or disappearing.
`invalidate()` on the save paths stays as a backstop for a filesystem whose mtime is too coarse to tell two quick writes apart.

**Analysis status label moves its program resolution off the Qt thread.** `analysis_core::status`'s PATH lookup, composer.json parse, and — on a WSL project root — `ExecHost`'s `wsl.exe` probe (`process_exec::host::resolve_program`) used to run inline inside `AnalysisService::analyzer_rows()`, called synchronously from `status_bar.cpp`'s `projectOpened` handler. `AnalysisService` gains `refresh_analyzer_status_async` (fire-and-forget, mirroring `ProjectTreeModel::openFolder`'s own shape) and a new signal, `analyzerStatusReady(Vec<FfiAnalyzerRow>)`: the worker computes the same rows `analyzer_rows()` would (both now call a shared `build_analyzer_rows` free function), and the completion closure drops the result if the project is no longer the open one by the time it lands (`current_project_root()` re-checked on the Qt thread, the same stale-root guard `ProjectTreeModel::start_watcher_async`/`AppSession::install_watcher` already use). `analyzer_rows()` itself is unchanged and stays synchronous — the Analysis settings page reads it on demand, an occasional action where "always fresh, even for a plugin enabled seconds ago" is worth a small parse, unlike every `projectOpened`. `status_bar.cpp` now triggers `refreshAnalyzerStatusAsync()` on `projectOpened` and renders from the last `analyzerStatusReady` snapshot instead of calling `analyzerRows()` itself.

**Left synchronous, and why.** `DebugService::load_breakpoints` (`.ide/local/breakpoints.toml`, wired to `projectOpened` in `main_window.cpp`) stays on the Qt thread: it is one small machine-local file with no process spawn, the same size class `push_recent_project`'s settings.toml read already runs synchronously (ADR-0062 Decision 1's own "not the bottleneck" call). `RunService::detect_configurations` and `ContainerService::refresh_all` were already off-thread or backed by a worker before this PR and needed no change.
