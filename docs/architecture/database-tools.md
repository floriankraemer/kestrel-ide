...
`ExchangeService`, the exchange/dump/diagram/compare **dialogs**, and the Run-dock hookup for `dump`'s `spawn` are F5b/F6b — `DatabaseService`, `SessionWorker`, `ConsoleService`, `ResultProvider`, `DriverInstallService` and every other `cpp/` view below remain target design.*
