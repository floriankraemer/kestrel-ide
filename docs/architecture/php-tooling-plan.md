...
| Task | Status | Commit |
|---|---|---|
| E1 — a stub analyzer binary (a script that prints fixture XML), on the `lsp-core::bin::stub_server` precedent, so CI needs no PHP | open | |
...
| 4 | A `temp-copy` beside the original can be committed or picked up by the tool's own file walker. | The copy is a dotfile with a per-run suffix, removed on completion *and* on cancellation, and `.ide/.gitignore` covers the pattern. |
