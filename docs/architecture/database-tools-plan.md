...
|---|---|---|---|
| Data sources | 30+ complete / 20+ basic DBMS; SSH/SSL/proxy; password/Kerberos/IAM/pgpass; read-only; groups + colour; project vs global; driver auto-download; `dataSources.xml` without passwords | **As delivered**: native PostgreSQL, MySQL/MariaDB, SQLite, MongoDB, Redis, Cassandra/Scylla. ADBC with a pinned, IDE-managed download: DuckDB, Snowflake, BigQuery. ADBC/ODBC `install-hint` only (no pinnable artifact found, F8.3 spike): SQL Server, ClickHouse, Trino. ODBC generic row: any system DSN (Oracle/Databricks/Presto/HANA/Teradata/Redshift/Spark/SingleStore/Exasol/Druid/Flight SQL are reachable this way if a system driver exists, not their own manifest rows). SSH tunnel (CLI first, agent/key/config/password/ProxyJump; in-process `russh` fallback for password auth or no CLI), TLS modes (Postgres real; Cassandra plaintext-only), password auth; read-only; groups + colour; global/project TOML; secrets in keychain. Kerberos/IAM/proxy deferred. | F1, F7, F8, FZ |
| Database tool window | tree db/schema/table/view/mat view/routine/sequence/trigger/index/constraint/column/role/user; pattern + type filters; grouping; speed search; refresh/force; introspection levels; Go to DDL; Jump to console; Edit data; SQL Generator; rename/drop/truncate/comment; diagrams; import/export entries | all except the quick-doc popup, "scroll from editor", speed-search and a themed icon mask set (all recorded debt, see database-tools.md §11) | F2 |
| Query console | per source, many; schema switcher; run statement/selection/file; Output + Result tabs; tx auto/manual; error policy stop/ignore/ask; history; cancel; run SQL file config | all; consoles stored as `.sql`/`.mongodb`/`.redis`/`.cql` files by family; `sql-script` run-configuration kind (no cancel/rerun, recorded debt) | F3 |
| Result grid / data editor | table/tree/text/transpose; paging; WHERE/ORDER BY; inline + value editor; NULL/DEFAULT; add/delete/clone; submit/revert; DML preview; FK navigation; record view; aggregates; column visibility | Table/Transpose/Text/Record view modes (Record covers the nested Document/Array/JSON case JetBrains' tree view targets, no separate tree widget), forward + reverse FK navigation, exact + estimated aggregates, column-visibility menu; everything else delivered; a geo viewer and charts stay out of scope | F3, F4 |
| Import/export | CSV/TSV/JSON/SQL insert+update/HTML/Markdown/XLSX; clipboard/file/console; CSV import mapping; pg_dump/mysqldump; copy table between sources | all seven export formats, CSV/XLSX import mapping, copy table between sources; dump *and* restore (F6c) via `process-exec` to the DBMS' own tool (`pg_dump`/`pg_restore`, `mysqldump`/`mysql`, `mongodump`/`mongorestore`, `sqlite3 .dump`); XLSX via `rust_xlsxwriter`/`calamine`; own job-progress signals rather than the Run dock (recorded debt) | F5 |
| Diagrams | ER diagram of schema/table | Mermaid `erDiagram` generated from the schema model with real cardinality (F6c), rendered by the existing preview dock (ADR-0033) both as a raster dialog and a focusable, tab-following `.mmd` tab | F6 |
| Schema compare + migration (Ultimate) | | object-level diff (incl. structural index/constraint changes, F6c) → migration script + per-object DDL pairs on `DiffView` (ADR-0030); routine/view diffs stay text-only (recorded debt) | F6 |
| Data compare (Ultimate) | | key-aligned row diff as canonical TSV per side on `DiffView` (grid diff = recorded debt) | F6 |
| Create/modify object dialogs (Ultimate) | | table/column/index/user editors (F4.4/F4d) emitting a DDL preview before running; no FK editor in the create-table dialog beyond an inline `FOREIGN KEY` clause, Modify Table is single-column not a multi-row grid (recorded debt) | F4 |
| SQL assistance | dialect-aware completion, inspections, formatting, navigation | native Rust provider over the introspected schema (`sqlparser` 0.63 + `sqlformat` 0.5), surfaced through the existing completion/diagnostics seams — not LSP; selection-scoped "Format SQL" not wired (the FFI carries no selection range yet, recorded debt) | F3 |
| NoSQL | MongoDB, Redis, Cassandra complete | collections/keys/keyspaces in the same tree; Record grid mode for document data; Mongo `runCommand` JSON + `db.coll.find(...)` sugar (no JS engine), RESP console, CQL console (no CQL parser, keyword+schema completion only); Cassandra/Scylla plaintext-only (recorded debt, see database-tools.md §11) | F7 |
...
*This is P0's original design sketch (types/signatures as first drafted), kept for its design rationale.
`database-tools.md` §2 is the truth-up against the code as it actually shipped — read that one for the real file list; one known-stale detail here: `db-drivers` never grew its own `tunnel.rs` (the real file is `ssh.rs`, holding `RusshTunnel`; `Tunnel`/`CliTunnel` live in `db-core::tunnel` as designed) — `mysql.rs` itself landed in phase FZ, matching this sketch.*
...
| F4.3 follow-up — reverse FK navigation ("Show referencing rows…"), transpose/tree/text view modes, the typed row accessor (`rowValues`) db-exchange needs for "Copy as" fidelity, and the C++ footer/context-menu wiring for the aggregate/nav bridge calls above — see database-tools.md §11 | done — typed rows/hex/FK-nav-context-menu closed by F4c; view modes, aggregate footer, reverse FK nav closed by F4d below | |
| F4c — result grid view modes, typed rows, FK/aggregate UI wiring: typed rows (`ResultProvider::rowValues`, JSON-per-row, closes F5b's typed-export gap), binary/hex value-editor mode, FK-navigation context menu + `F4` shortcut (`cellNavigation`/`goToNavTarget`), and the `resultTab_` routing bug both that path and `applyClauses` shared | partial — view modes (Table/Transpose/Text/Record + column visibility) and the aggregate footer UI (`aggregateExact`/`aggregateComputed` wiring) not done this pass, reverse FK nav and F4.4 still fully open — closed by F4d below | 3157f6a, 7d444fa, 29b24b2 |
| F4d — result grid view modes (Table/Transpose/Text/Record + column visibility, Mongo defaults to Record), aggregate footer (`aggregateExact`/`aggregateComputed` with a Rust-decided exact-vs-fetched-rows-only fallback), F4.4 object dialogs (create/modify table, add/alter/drop column, create index, create user, per dialect, adversarial-identifier tested), reverse FK navigation ("Show referencing rows…") | done — scope cuts: Modify Table is single-column (add/modify/drop), not a multi-column grid; a `CREATE TABLE`'s indexes are a separate Create Index action, not inline; no column-visibility/view-mode persistence across a result change; reverse-nav's whole-schema introspect is not cached (re-fetched per click) — see database-tools.md §4/§11 | e154b6d |
| F4.4 — create/modify object dialogs (table/column/index/FK/user) | done — see F4d above | e154b6d |
...
| F5.3 — dump via `process-exec`; restore (F6c: `restore_command` + `spawn_with_stdin`) | done (dump and restore both real; `ExchangeService`'s own job signals rather than the Run dock — deviation recorded in database-tools.md §4/§11) | `03dd760`, F5b UI `79674cc`, F6c restore `0685470`/`a2fe428`/`2ca4a67` |
...
| Task | Status | Commit |
|---|---|---|
| F6.1 — ER diagram → Mermaid `erDiagram`, real cardinality (F6c), previewable in its own dialog *and* the tab-following Preview dock (F6c) | done | `8720907`, F6b UI `79674cc`, F6c `12d5b58`/`26cd294` |
| F6.2 — schema compare → `MigrationScript` on `DiffView`; structural index/constraint diff (F6c) | done | `23433f7`, F6b UI `79674cc`, F6c `12d5b58` |
...
| F7.5 — `db_core::tunnel::Tunnel` trait; `RusshTunnel` in `db-drivers` (password/passphrase, known-hosts prompt) | done — F7b added the UI half (`hostKeyPrompt`/`acceptHostKey`, `known_hosts::append`); F7c wired the tunnel itself into every connect path (`ui_shell::bridge::database::open_session`), so `HostKeyUnknown` now genuinely reaches the prompt from a real "Test connection"/tree connect (database-tools.md §4/§9/§11's F7b debt row paid off) | `d3ae943 (db-core)/90a412e (db-drivers)`, F7b: `e52d9fa` (db-drivers `ssh.rs`), host-key-prompt UI `ae00213`, F7c: `2356eb1` (db-core, ui-shell) |
| F7.6 — document grid mode | done — closed by F4d's Record view mode, not a dedicated F7b toggle: `ResultProvider::resultModes` defaults to Record for a Mongo-shaped result (one column, a `Document` per row, `has_nested_structure`), with Table/Transpose/Text also available; `flatten_documents` (table-mode column flattening) and `Value::display`'s compact JSON-ish cell text were already in place from F7a | e154b6d |
| F7.7 — family-aware Data Source dialog (the gap F7b left relabel-only) | done, scoped down — `db_core::console::extra_fields(family)` descriptor (Mongo replica set, Redis TLS toggle, Cassandra local datacenter — the only fields any family needs beyond F7b's relabelled common ones), `DataSourceEditor::{extraFields,option,setOption}`, `data_source_dialog.cpp`'s generic Options-tab builder; a fully generic form-builder for every field (host/port/database included) deliberately not built (database-tools.md §4/§11: YAGNI, nothing needed it yet) | F7c: `a899b78` (db-core, settings-model, ui-shell) |
...
### FZ — MySQL/MariaDB native driver + idle connection close

Two items the F9 truth-up found missing rather than merely deferred: MySQL/MariaDB had no driver crate at all, and `idle_close_minutes` was a setting with no code behind it.

| Task | Status | Commit |
|---|---|---|
| FZ.1 — `db-drivers::mysql` (feature `mysql`, default on): `mysql_async` 0.37, `rustls-tls`+`ring`+pure-Rust `flate2` (no `aws-lc-rs`, no C zlib); connect/execute/streaming/cancel (`KILL QUERY` on a second connection)/introspection at all three levels/`ddl_of` via `SHOW CREATE`; `mysql`/`mariadb` `database-drivers` manifest rows (same native id, family `mysql`) and a `mysql` `sql-dialects` row + keyword asset; `plugin-host`'s row-count assertions updated | done | ba57aa1, 88c6533 |
| FZ.2 — real-server tests: `docker/db-compose.yml` gains `mysql:8.4`/`mariadb:11`; `make test-db`'s `db-ci` exercises both (connect/execute/introspect, a 1M-row recursive-CTE stream, `KILL QUERY` cancel of a `SELECT SLEEP(30)`) | done | e118697 |
| FZ.3 — idle connection close: `SessionWorker`'s own `recv_timeout` loop drops an otherwise-quiescent Auto-mode connection (and its tunnel) after `idle_close_minutes`, reopening lazily on the next command through the same reconnect path (`db_core::session::Session::replace_connection`, additive); `SessionWorker::spawn` (every existing caller) unaffected (`idle_close_minutes: 0`) | done — the timer itself, unit-tested with a fake connection and a 1-second setting; wiring the production `reconnect` closure into `bridge::database::service.rs`'s `open_session` call site is left for the integrating session (out of this phase's file list, see database-tools.md §5) | 25b78ec |

## 10. Verification (F9 truth-up: what actually runs today, see `docs/architecture/db-integration.md` for the full account)

- Per PR: `make test`, `make lint` (incl. the strict `cargo tree --all-features -i aws-lc-rs` gate), the layering greps above, the file-size gate, patch coverage ≥ 80%, `e2e_database` + `e2e_database_console` on SQLite (two flows, budget 14/16).
- Nightly/on demand (`docs/architecture/db-integration.md`): no dedicated `linux-db` image stage yet — `make test-db` reaches `docker/db-compose.yml`'s services (`postgres:17`, `mysql:8.4`, `mariadb:11`, `mongo:8`, `redis:7`, `scylladb/scylla:2026.2`, `linuxserver/openssh-server` for the SSH tunnel test) from inside `linux-builder` itself over `--network host`; `db-ci` = `cargo nextest run -p db-drivers --features db-integration` plus a second `--run-ignored only ssh` pass for the sshd-backed tunnel test. SQL Server (`mssql/server:2022`) has no compose service yet, `db-driver-adbc`/`db-driver-odbc` are not yet wired into `db-ci`, `.github/workflows/nightly.yml` has no `db-integration` job yet, and `make e2e-db` does not exist — all three remain open follow-up work, not shipped.
...
## 10a. Delivered (F9 summary)

Seven new Qt-free crates (`secret-store`, `db-core`, `db-sql`, `db-exchange`, `db-drivers`, `db-driver-adbc`, `db-driver-odbc`), the `database-tools` built-in plugin, `tool-windows`/`settings-pages`/`database-drivers`/`sql-dialects` contribution points shared with `jvm-build-tools`/`containers`, and the full `ui-shell::bridge::database` + `database_*.cpp`/`result_*.cpp`/`db_*.cpp`/`data_source_*.cpp`/`driver_install_dialog.cpp` view layer.

Delivered against the parity matrix (§2): data sources over native PostgreSQL/SQLite/MongoDB/Redis/Cassandra-Scylla, ADBC (DuckDB/Snowflake/BigQuery pinned, SQL Server/ClickHouse/Trino install-hint-only) and ODBC (any system DSN); the Database tool window with introspection levels, Go to DDL, SQL Generator, rename/drop/truncate/comment; query consoles as family-aware files with run/cancel/history/tx modes; a windowed result grid with Table/Transpose/Text/Record view modes, an aggregate footer, forward+reverse FK navigation; a full data editor (inline + value editor incl. hex mode, NULL/DEFAULT, add/delete/clone, DML preview, submit/revert); create/modify object dialogs (table/column/index/user); all seven export formats, CSV/XLSX import, dump *and* restore, copy-table; an ER diagram, schema compare + migration script, and data compare, all riding the existing preview/diff infrastructure; SQL/Mongo-shell/RESP assistance (completion, inspections, formatting, navigation) injected directly into `LanguageService`; an SSH tunnel (CLI-first, `russh` fallback) wired into every connect path with host-key consent; and the full security posture of ADR-0061.

Not delivered, tracked as open follow-up rather than silently dropped: a dedicated `linux-db` Docker image stage and `make e2e-db`, SQL Server's nightly compose service, and a from-scratch Windows cross-build of the tree this plan actually shipped (§13 is P0's own spike only — see the Windows note below); MySQL/MariaDB — the one gap this section used to name — was closed in phase FZ (native driver, both compose services, idle connection close). Every other gap is recorded, with its own trigger, in `database-tools.md` §11's consolidated debt table.

Test counts at the last full gate on this branch: 3931 unit tests workspace-wide (`cargo nextest run --workspace`), plus 84 `db-integration`-gated real-server tests and 1 explicitly re-run sshd-tunnel test (85 total, `db-ci`) against Postgres/Mongo/Redis/Scylla/sshd, and 14 of the per-PR E2E budget's 16 flows now landed workspace-wide (`e2e_database`/`e2e_database_console` are Database Tools' own two). Patch coverage on the seven new Qt-free crates stayed ≥ 80% per phase, gated the same way as every other crate.
...
| W5 | **F9** | W4 merged | 1 agent: architecture-doc truth-up, i18n sweep, ADRs → Accepted, Windows manual matrix. F9's own agent is docs-only and has no mandate to rebuild `ide-windows-builder` or run a Windows cross-build — both remain pending, recorded as a manual step for the main session (§10a, F9.5). Main session: final `cargo clean` of dead worktree artifacts, prune worktrees/branches/containers. |
