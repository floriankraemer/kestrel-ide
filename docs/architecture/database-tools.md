...
**Known gap, found not fixed**: `db-drivers`' `SqliteConnection`/`PgConnection::execute` both eagerly drain the *entire* `Rows` result into memory before returning (`RowStream::next_batch` only re-chunks an already-fully-materialised buffer) — a pre-existing P0/F1-era gap, out of this phase's file list, that defeats the "first batch ≤ 500 ms, later pages cheap" NFR at real scale; §11 tracks it as new debt for whichever phase touches those drivers next.
...
`db-drivers`' eager `Rows`-materialisation gap this phase found (§4's F3 implementation note) is not fixed — out of this phase's file scope, needs its own task against `db-drivers`.
...
F3.7 (`database_completion`/`database:inspections`/"Format SQL"/"Go to DDL" intentions) was not started this phase.
