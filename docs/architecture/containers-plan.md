...
**C5 — Run configurations (Image / Containerfile / Compose) and Compose nodes.** ✅ shipped as described below, with the UI scope cuts this paragraph now records.
`RunConfigSetting.kind` + `ContainerImageRunSetting`/`ContainerfileRunSetting`/`ComposeRunSetting` (every JetBrains option from the three tables, same names, in a new `app_config::container_run`); `container_core::run_config` argv compilers (every option → its documented flag, both engines) + `preview()` (shell-quoted) + `compose_services` (`compose config --services`); `run-core::RunConfigExt::to_launch_spec_in` dispatches by kind through a new `container_run` module (`program`/`args` derived from the sub-table, never stored) — connection resolution threads `app_config::ContainerSettings` through a new `MacroContext::containers` field (the smallest change that reaches every `to_launch_spec_in` call site without a second method); Containerfile with `run_built_image` set gets an auto `BeforeLaunchTask::ExternalTool{docker build …}` via `before_launch::tasks_of_with_containers` (build output in the Build dock, the IDE's before-launch convention) — with it unset, the configuration's own launch *is* the build, usable as another configuration's before-launch `RunConfiguration` task; `stop_command`/`down_command` back a compose console's Stop/Down actions; `detect.rs` suggests a `containerfile`/`compose` configuration per conventional `Dockerfile`/`Containerfile`/compose file name found at the project root.
`run_config_dialog.cpp` gains an Add ▸ Containers submenu (Container Image / Containerfile / Compose) and a live "Command preview" bound to `RunConfigEditor::commandPreview`.
**Scope cut, recorded rather than silently dropped**: the per-kind pages are one JSON text editor (`FfiRunConfig::container_json`) rather than per-field port/mount/env/build-arg/compose-file table widgets, a "Modify options" disclosure menu, a Server combo backed by `connections()`, or a Services multi-picker wired to a table (`RunConfigEditor::composeServices` exists and is called from Rust tests, but no widget calls it yet); Compose tree nodes (Start All/Stop/Down/Scale/Jump to Source), the Dockerfile gutter's Build/Run/New-configuration popup, and replacing C4's `createContainerQuick` with the run-config dialog are deferred to a follow-up task — `ContainerService`/`containers_panel.cpp` (C2–C4) are unchanged by this task. `stop_command`/`down_command` are reachable only via `RunService::composeDown`/the existing Stop button, not yet a tree action. ADR-0056 (extends ADR-0032/ADR-0039).
Tests: table-driven argv for every option × both engines, preview string quoting, the SELinux `:z` rule, the shell-word splitter, Containerfile build vs. run-built-image dispatch, `tasks_of_with_containers`' auto build task, `detect`'s Dockerfile/compose suggestions, TOML round-trips for all three sub-tables.
